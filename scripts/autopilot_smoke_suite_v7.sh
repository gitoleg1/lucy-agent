...
# ניהול ENV: שומרים גיבוי, בונים קובץ זמני לשלבים השונים, ובסוף משחזרים
ORIG_ENV="$(mktemp)"
...
# 👇 תיקון חשוב: בודקים type ישירות על ה־JSON, לא על טקסט שטוח
st_type="$(jq -r '.audit[-1].data.stdout_tail | type' <<<"$resp")"
er_type="$(jq -r '.audit[-1].data.stderr_tail | type' <<<"$resp")"
